...
public class BukkitUtil {
	...
	public static PluginCommand constructCommand( String id ) {
		...
		try {
			command = PLUGINCOMMAND_CONSTRUCTOR.newInstance( id, SpaceUtil.getPlugin() );
	...
	public static double getMaxHorizontalSpeedFor( Player player ) {
		...
		if ( player.hasPotionEffect( PotionEffectType.SPEED ) ) {
			...
			speed *= 1 + ( effect.getAmplifier() + 1 ) * .2;
		}

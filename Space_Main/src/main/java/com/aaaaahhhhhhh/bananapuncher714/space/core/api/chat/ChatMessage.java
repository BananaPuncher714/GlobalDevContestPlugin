package com.aaaaahhhhhhh.bananapuncher714.space.core.api.chat;
...
import com.aaaaahhhhhhh.bananapuncher714.space.core.util.MDChat;
import com.aaaaahhhhhhh.bananapuncher714.space.core.util.MessageUtil;
...
public class ChatMessage implements Cloneable, Serializable {
	...
	public TextComponent toMDChat() {
		TextComponent textComponent = new TextComponent();

		for ( ChatComponent component : components ) {
			TextComponent subComponent = new TextComponent( component.getText() );
			subComponent.setBold( component.isBold() );
			subComponent.setColor( MessageUtil.toMDColor( component.getColor() ) );
			subComponent.setItalic( component.isItalic() );
			subComponent.setObfuscated( component.isRandom() );
			subComponent.setStrikethrough( component.isStrikethrough() );
			subComponent.setUnderlined( component.isUnderline() );
			
			ClickAction clickAction = component.getClickAction();
			if ( clickAction != null ) {
				subComponent.setClickEvent( new ClickEvent( ClickEvent.Action.valueOf( clickAction.getAction().name() ), clickAction.getMessage() ) );
			}
			
			HoverAction hoverAction = component.getHoverAction();
			if ( hoverAction != null ) {
				subComponent.setHoverEvent( new HoverEvent( HoverEvent.Action.valueOf( hoverAction.getAction().name() ), new BaseComponent[] { MDChat.getMessageFromString( hoverAction.getMessage() ) } ) );
			}

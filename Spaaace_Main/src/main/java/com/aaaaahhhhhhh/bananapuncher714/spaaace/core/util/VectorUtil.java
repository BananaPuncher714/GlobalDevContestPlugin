...
import com.aaaaahhhhhhh.bananapuncher714.spaaace.core.api.util.AABB;
...
public final class VectorUtil {
	...
	public static CollisionResultEntity rayIntersect( Entity entity, Location origin, Vector ray ) {
		...
		Location entLoc = SpaaaceUtil.getPlugin().getLocationTracker().getLocationOf( entity.getUniqueId() );
		if ( entLoc != null ) {
		...
		if ( entry != null ) {
			return new CollisionResultEntity( SpaaaceUtil.getEntity( entity ), entry, entryFace, exit, exitFace );
		}
		return null;
	}
...
//		double pitchDiff = pitch - ( rand.nextDouble() * 2 * pitch );
	...
	public static boolean isHeadshot( CollisionResultEntity intersection ) {
		...
		if ( entity instanceof GunsmokeEntityWrapperLivingEntity ) {
			...
			Location entityLocation = SpaaaceUtil.getPlugin().getLocationTracker().getLocationOf( entity.getUUID() );
